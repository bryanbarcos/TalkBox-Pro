...
/**
 * Class SoundEngine provides functionality to process sound files. Sounds can
 * be loaded, played, paused, resumed, and stopped.
 *
 * The sound engine can play multiple sounds simultaniously, but only the last
 * sound loaded can be controlled (pased, resumed, etc.).
 *
 * The sound engine accepts files in WAV, AIFF, and AU formats (although not all
 * WAV files - it depends on the encoding in the file).
 */
public class SoundEngine {
	// the following three fields hold information about the sound clip
	// currently loaded in this engine
	public Clip currentSoundClip = null;
	...
	/**
	 * Load and play a specified sound file. If the file is not in a recognised file
	 * format, false is returned. Otherwise the sound is started and true is
	 * returned. The method returns immediately after the sound starts (not after
	 * the sound finishes).
	 *
	...
	/**
	 * Stop the currently playing sound (if there is one playing). If no sound is
	...
	/**
	 * Pause the currently playing sound. If no sound is currently playing, calling
	...
	/**
	 * Resume the currently paused sound. If no sound is currently paused, this
	...
	public void setVolume(int vol) {
		...
		try {
			FloatControl volControl = (FloatControl) currentSoundClip.getControl(FloatControl.Type.MASTER_GAIN);
	...
	public boolean isPlaying() {
		if (currentSoundClip.isActive() == false) {
			return false;
		} else {
			return true;
		}
	...
	public boolean loadSound(File file) {
		...
		try {
			...
			currentSoundDuration = (int) (currentSoundClip.getBufferSize()
					/ (currentSoundClip.getFormat().getFrameSize() * currentSoundClip.getFormat().getFrameRate()));
			return true;
		} catch (Exception ex) {
			currentSoundClip = null;
			return false;
